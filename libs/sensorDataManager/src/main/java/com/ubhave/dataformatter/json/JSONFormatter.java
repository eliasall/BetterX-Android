...
import com.ubhave.datahandler.UIDGenerator;
...
{
	private final static String SENSOR_TYPE = "sensor";
	private final static String SENSOR_DATA = "sensorData";
	private final static String SENSE_TIME = "senseStartTime";
	private final static String SENSE_TIME_MILLIS = "timestamp";
	private final static String UNKNOWN_SENSOR = "unkown";
	...
	public JSONObject toJSON(final SensorData data) throws DataHandlerException
	{
		...
		if (data != null)
		{
			try
			{
				...
				addSensorSpecificData(d, data);
				SensorConfig config = data.getSensorConfig();
				if (config != null)
				{
					addGenericConfig(d, config);
					addSensorSpecificConfig(d, config);

					json.put(SENSOR_DATA, d);
				}
			}
	...
	protected void addGenericData(JSONObject json, SensorData data)
	{
		try
		{
			String userId = UIDGenerator.getUID(applicationContext);
...
//			}
		...
		// SimpleDateFormat("HH:mm:ss:SSS dd MM yyyy Z z", Locale.US);
		try
		{
